...
**Node attribution.** The score is instance-level, but for a flagged instance the verdict names *which* node when its dominant feature (`topIssue`) is **node-localizable**. `BaselineFeatureExtractor.attribute(pose)` returns the culprit node(s) — the argmax/argmin that `extract()`'s reductions discard — for `max_edge_zscore` (both endpoints), `max_angle_zscore` (the joint), `max_pairwise_zscore` (the pair), `max_centroid_distance` (the "isolated" node), `min_symmetry_consistency` (the worst pair), and `has_isolated_invisible` (the invisible node whose neighbors are all visible). `visibility_pattern_score` is localized separately by `VisibilityModel.worstNode` — the most-blamed node in the co-visibility matrix (a peer that is expected-co-visible `P>0.9` yet absent, or rarely-co-visible `P<0.1` yet present). `qcStore.anomalyWorstNode` (lazy, cached; uses the `fx` returned by `computeAnomalyUnit`) resolves `topIssue`'s feature to its node — via the baseline extractor, or the visibility model for `visibility_pattern_score` — and feeds `faultyNodeFor` → the red ring + sidebar name + zoom. Whole-instance features (`mean_*`, `bbox/hull/nn`, `visibility_rate`) are intentionally **not** localized → no node, generic verdict. A named *invisible* node (isolated-invisible, or a visibility-pattern "expected but absent" node) has no coordinates, so it cannot be ringed/zoomed (zoom falls back to the instance box).
